        ...
        <div class="table-responsive text-nowrap">
            <table class="table">
                ...
                <tbody class="table-border-bottom-0">
                    ...
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $value->artist->first_name }} {{ $value->artist->last_name }}</td>
                        <td>{{ $value->clips_count ?? 1}} Clips</td>
                        <td>{{ $value->size ?? 0 }} GB</td>
                        <td>{{ $value->duration }}</td>
                        <td>
                            <div class="d-flex justify-content-start align-items-center">
                                <a class="btn" href="{{ url('video-clip/' . $value->id) }}"><i class="bx bx-show"></i></a>
                                <span data-bs-toggle="modal" data-bs-target="#editmusicModal{{ $value->id }}">
                                ...
                                <form action="{{ url('video-clips/' . $value->id) }}"
                                ...
                                <div class="modal fade" id="editmusicModal{{ $value->id }}" aria-modal="true" role="dialog">
                                    <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form id="editForm{{ $value->id }}" class="edit-form" method="POST"
                                                    action="{{ url('video-clips/' . $value->id) }}"
                                                    enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="hidden-inputs">
                                                        <input type="hidden" name="video_paths[]"
                                                            value="{{ $value->video_path }}"
                                                            data-path="{{ $value->video_path }}">
                                                    </div>
                                                    <div class="row">
                                                            ...
                                                            <div class="row g-3">
                                                                <div class="col-md-12">
                                                                    ...
                                                                    <select class="form-select"
                                                                        ...
                                                                        @foreach($music_category as $category)
                                                                            <option value="{{ $category->id }}" {{ $category->id == $value->category_id ? 'selected' : '' }}>
                                                                                {{ $category->name }}
                                                                            </option>
                                                                        @endforeach
                                                                    </select>
                                                                ...
                                                                <div class="col-md-12">
                                                                    ...
                                                                    <select class="form-select"
                                                                        ...
                                                                        @foreach($artists as $artist)
                                                                            <option value="{{ $artist->id }}" {{ $artist->id == $value->artist_id ? 'selected' : '' }}>
                                                                                {{ $artist->name }}
                                                                            </option>
                                                                        @endforeach
                                                                    </select>
                                                                    ...
                                                                    <div class="card">
                                                                        ...
                                                                        <div class="card-body">
                                                                            <div class="dropzone needsclick dz-clickable dz-started"
                                                                                action="/" id="dropzone-video{{ $value->id }}">
                                                                                <div class="dz-message needsclick">
                                                                                    Drop files here or click to upload
                                                                                </div>
                                                                                <div class="row" data-path="{{ $value->video_path }}">
                                                                ...
                                                                <div class="col-md-12">
                                                                    ...
                                                                    <select class="form-select"
                                                                        aria-label="Default select example"
                                                                        name="status">
                                                                        <option value="0" {{ $value->status == 0 ? 'selected' : '' }}>UnPublish</option>
                                                                        <option value="1" {{ $value->status == 1 ? 'selected' : '' }}>Publish</option>
                                                                    </select>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <script>
                                                    ...
                                                    dropZoneInitFunctions.push(function() {
                                                        ...
                                                        const dropzoneMulti = new Dropzone('#dropzone-video{{ $value->id }}', {
                                                            url: '{{ url("file/upload") }}',
                                                            ...
                                                            removedfile: function(file) {
                                                                ...
                                                                $.ajax({
                                                                    url: '{{ url("video-clips/" . $value->id . "/clip") }}',
                                                                    method: 'delete',
                                                                    headers: {
                                                                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                                                                    },
                                                        ...
                                                        $("document").ready(() => {
                                                            var path = '{{ url("storage/" . $value->video_path) }}';
                                                            let name = '{{ $value->video_name }}';
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="submit" form="editForm{{ $value->id }}" class="btn btn-primary"
