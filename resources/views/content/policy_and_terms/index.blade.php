                    ...
                    <div class="tab-pane fade show {{ $loop->first && !old('tab') ? 'active' : '' }} {{ old('tab') === "tab{$section->id}"  ? 'active' : '' }}" id="tab{{ $section->id }}" role="tabpanel">
                        <form action="{{ route('policy_and_terms.saveFileds') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" name="privacy_policy" rows="6">{{ old('privacy_policy', $section->privacy_policy) }}</textarea>
                            </div>
                            {{-- <div class="mb-3">
                                <label>Disclaimer</label>
                                <textarea class="form-control" name="disclaimer" rows="6">{{ old('disclaimer', $section->disclaimer) }}</textarea>
                            </div> --}}
